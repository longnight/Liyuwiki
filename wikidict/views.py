...
from secretballot.middleware import SecretBallotIpUseragentMiddleware, SecretBallotMiddleware
from secretballot.models import Vote
from script import term_uid, definition_uid, confidence, term_pinyin, query_previous, query_next
...
def index(request):
    ...
    termlist = Terms.objects.all().filter(terms_definitions__show=True).distinct()
    ...
    context = {'sform': sform, 'latest_created': latest_created, 'latest_voted': latest_voted,
               'author_count': author_count, 'terms_count': terms_count, 'defs_count': defs_count,
...
def detail(request, term_uid):
    ...
    if related_terms_cache == None:
        ...
        if related_terms:
            the_cache.set((str(term_uid) + '_' + 'related'), related_terms, 60 * 60 * 12)
        else:
            the_cache.set((str(term_uid) + '_' + 'related'), 'no_related', 60 * 60 * 12)
    ...
    if also_voted_term_cache == None:
        v_t = voted_term(term.id)
        if term in v_t:
            v_t.remove(term)
        if v_t:
            the_cache.set((str(term_uid) + '_' + 'also_voted'), v_t, 60 * 60 * 2)
        else:
            the_cache.set((str(term_uid) + '_' + 'also_voted'), 'non', 60 * 60 * 2)
    elif also_voted_term_cache == 'non':
        v_t = []
    else:
        v_t = also_voted_term_cache

    context = {'related_terms': related_terms, 'deflist': deflist, 'sform': sform, 'term': term,
               'previouslist': previouslist, 'recent_kw': list(recent_kw),
               'nextlist': nextlist, 'visited_rank': visited_rank, 'score_rank': score_rank,
...
def detail_single(request, t_uid, d_uid):
    ...
    sform = SearchForm2()
    context = {'term': term, 'sform': sform, 'definition': definition, 'visited_rank': visited_rank,
               'score_rank': score_rank, 'latest_vote': latest_vote, 'is_online': is_online}
